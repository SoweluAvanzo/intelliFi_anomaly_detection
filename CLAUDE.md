...
- **Dead Etherscan keys poison the whole IP (2026-09-01).** A retired/invalid key returns "too many invalid api key attempts / Invalid API Key (#err2)" on its *first* request, and that trips a **per-IP** penalty: for the next ~30–45 min even *valid* keys from that IP fail intermittently. Vetted-valid set is **ETHERSCAN_KEY, 4, 5, 6**; **dead: 2, 3, 7** (commented out in `.env.dune`). Guards now enforce this: `config.CRAWL_KEYS` is a hard allowlist by env-NAME (override `INTELLIFI_CRAWL_KEYS`); `scripts/10_fetch_v2_tape.py` refuses a non-allowlisted `--api-key-env`, and `scripts/10_relaunch_v2_gaps.py` refuses to start if *any* unvetted key is present in the env. **Never add a startup "validate keys" step** — validation itself hits the dead keys and re-arms the penalty; trust the list. `Polygonscan` has a circuit breaker: consecutive invalid-key responses are never retried and raise `IPPenaltyError` after `invalid_key_breaker=5`, stamping a cross-process marker `data/logs/.ip_penalty`. `Polygonscan.penalty_active(cooldown_s=2700)` returns minutes-remaining; the crawler and relaunch orchestrator pre-flight it and refuse to launch onto a warm IP (delete the marker to force). A poisoned IP needs **silence, not retries** — relaunch loops (incl. the box `entrypoint.sh`) must check `penalty_active` before relaunching.
- **v2 tape chunks can overlap (2026-08-31).** The resumable/rebalanced crawl (multiple crawlers on races-assigned ranges, key relaunches) writes differently-cut `blocks=<A>-<B>/part.parquet` files over the same blocks, so a fill can appear in >1 chunk. `(tx_hash, evt_index)` is the unique key of an `OrderFilled` log — **always dedup by it when reading the tape directly** (`QUALIFY row_number() OVER (PARTITION BY tx_hash, evt_index)=1`). The `v2_raw` view (`INTELLIFI_SOURCE=tape_v2`) now does this automatically. Cohort 1 verified 0 duplicates (its analyses are unaffected); cohorts 2–6 and genesis have overlaps, so their analyses must dedup. Chunk-file boundaries are irrelevant to reads (analyses glob the union and filter by block/exchange/event).
