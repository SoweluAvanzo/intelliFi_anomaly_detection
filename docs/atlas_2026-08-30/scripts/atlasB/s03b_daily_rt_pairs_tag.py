...
for sub in ["rt_daily2","pairs_daily2"]: os.makedirs(f"{S}/{sub}", exist_ok=True)
con=duckdb.connect()
...
for f in files:
    ...
    if os.path.exists(f"{S}/pairs_daily2/{day}.parquet"): continue
    ...
    con.execute(f"""COPY (
      ...
      FROM fl GROUP BY ALL) TO '{S}/rt_daily2/{day}.parquet.tmp' (FORMAT PARQUET)""")
    ...
    con.execute(f"""COPY (
      ...
      FROM d GROUP BY ALL) TO '{S}/pairs_daily2/{day}.parquet.tmp' (FORMAT PARQUET)""")
    os.replace(f"{S}/rt_daily2/{day}.parquet.tmp", f"{S}/rt_daily2/{day}.parquet"); os.replace(f"{S}/pairs_daily2/{day}.parquet.tmp", f"{S}/pairs_daily2/{day}.parquet")
