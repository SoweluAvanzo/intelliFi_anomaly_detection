...
for sub in ["rt_daily2m","pairs_daily2m"]: os.makedirs(f"{S}/{sub}", exist_ok=True)
con=duckdb.connect()
...
for f in files:
    ...
    if os.path.exists(f"{S}/pairs_daily2m/{day}.parquet"): continue
    ...
    con.execute(f"""CREATE TEMP TABLE d AS SELECT asset_id, block_timestamp AS ts, price, maker, taker, taker_direction AS dir,
        usdc_amount AS usdc, usdc_amount/price AS sh, (fee_usdc>0) AS fee_flag, category AS cat, category_refined AS catr
        FROM read_parquet('{f}') WHERE price>0 AND usdc_amount>0""")
    # (b) taker orders -> one-step round trips
    con.execute(f"""COPY (
      WITH o AS (
        SELECT taker, asset_id, ts, dir, cat, catr, sum(usdc) usdc, sum(sh) sh, max(fee_flag) fee_flag
        FROM d GROUP BY ALL),
      w AS (
        SELECT *,
      ...
      fl AS (
        ...
        FROM w)
      SELECT DATE '{day.replace('_','-')}' AS d, cat, catr, fee_flag,
      ...
      FROM fl GROUP BY ALL) TO '{S}/rt_daily2m/{day}.parquet.tmp' (FORMAT PARQUET)""")
    ...
    con.execute(f"""COPY (
      ...
      FROM d GROUP BY ALL) TO '{S}/pairs_daily2m/{day}.parquet.tmp' (FORMAT PARQUET)""")
    os.replace(f"{S}/rt_daily2m/{day}.parquet.tmp", f"{S}/rt_daily2m/{day}.parquet"); os.replace(f"{S}/pairs_daily2m/{day}.parquet.tmp", f"{S}/pairs_daily2m/{day}.parquet")
