...
- **RQ1 no-arbitrage band — the strongest positive signal (Sample A); v2 directional-only.** On v1, the median deviation |Σ YES − 1| widens from 0.012 (fee-free) to 0.031 (fee), a +3.4–4.2 pp (percentage-point) increase robust to fixed effects (controls that absorb fixed differences across categories and periods) — a solid Sample-A result. On v2 cohort 1, fee families sit at **0.020, inside the predicted (0.012, 0.031] range** and above A's fee-free 0.012, i.e. the widening is **directionally consistent**. But this is **not yet a confirmed replication**: the *within-v2* fee>fee-free ordering does **not** hold (0.020 < 0.030) because the v2 fee-free negRisk control is tiny/illiquid (818 family-hours), and the **confirmatory H2 test is still pending** (`confirmatory_verdict.md` H2 = Pending; `class_contingency_h2.json` = deferred). Read as a suggestive single-cohort signal, not a certified result. Interpretation *if it holds*: fees make it cheaper to move a candidate's price out of line before arbitrage corrects it.
